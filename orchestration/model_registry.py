...
def _build_simple_cnn(_: str, num_classes: int) -> nn.Module:
    from trainers.simple_cnn import SimpleCNN

    return SimpleCNN(num_classes=num_classes)


def _build_resnet18(_: str, num_classes: int) -> nn.Module:
    from trainers.resnet18 import ResNet18

    return ResNet18(num_classes=num_classes)


def _build_mobilenet_v2(_: str, num_classes: int) -> nn.Module:
    from trainers.mobilenet_v2 import MobileNetV2

    return MobileNetV2(num_classes=num_classes)


def _build_googlenet(_: str, num_classes: int) -> nn.Module:
    from trainers.googlenet import GoogLeNet

    return GoogLeNet(num_classes=num_classes, aux_logits=False)


def _build_densenet121(_: str, num_classes: int) -> nn.Module:
    from trainers.densenet121 import DenseNet121

    return DenseNet121(num_classes=num_classes)


def _build_efficientnet_from_scratch(_: str, num_classes: int) -> nn.Module:
    from trainers.efficientnet_from_scratch import EfficientNet

    model = EfficientNet(num_classes=num_classes)
    from trainers.efficientnet_from_scratch import load_efficientnet_pretrained
    return load_efficientnet_pretrained(model, num_classes)


def _build_efficientformer_v2_from_scratch(_: str, num_classes: int) -> nn.Module:
    from trainers.efficientformer_v2_from_scratch import EfficientFormerV2

    model = EfficientFormerV2(num_classes=num_classes)
    from trainers.efficientformer_v2_from_scratch import load_efficientformer_v2_pretrained
    return load_efficientformer_v2_pretrained(model, num_classes)


def _build_fastervit_from_scratch(_: str, num_classes: int) -> nn.Module:
    from trainers.fastervit_from_scratch import FasterViT

    model = FasterViT(num_classes=num_classes)
    from trainers.fastervit_from_scratch import load_fastervit_pretrained
    return load_fastervit_pretrained(model, num_classes)
...
_EXACT_SPECS: dict[str, ModelSpec] = {
    ...
    "simple_cnn": ModelSpec(
        name="simple_cnn",
        train_module="trainers.simple_cnn",
        weights_key="simple_cnn",
        default_image_size=224,
        builder=_build_simple_cnn,
    ),
    "resnet18": ModelSpec(
        name="resnet18",
        train_module="trainers.resnet18",
    ...
    "mobilenet_v2": ModelSpec(
        name="mobilenet_v2",
        train_module="trainers.mobilenet_v2",
        weights_key="mobilenet_v2",
        default_image_size=224,
        builder=_build_mobilenet_v2,
    ),
    "googlenet": ModelSpec(
        name="googlenet",
        train_module="trainers.googlenet",
        weights_key="googlenet",
        default_image_size=224,
        builder=_build_googlenet,
    ),
    "densenet121": ModelSpec(
        name="densenet121",
        train_module="trainers.densenet121",
        weights_key="densenet121",
        default_image_size=224,
        builder=_build_densenet121,
    ),
    "efficientnet_from_scratch": ModelSpec(
        name="efficientnet_from_scratch",
        train_module="trainers.efficientnet_from_scratch",
        weights_key="efficientnet_from_scratch",
        default_image_size=224,
        builder=_build_efficientnet_from_scratch,
    ),
    "efficientformer_v2_from_scratch": ModelSpec(
        name="efficientformer_v2_from_scratch",
        train_module="trainers.efficientformer_v2_from_scratch",
        weights_key="efficientformer_v2_from_scratch",
        default_image_size=224,
        builder=_build_efficientformer_v2_from_scratch,
    ),
    "fastervit_from_scratch": ModelSpec(
        name="fastervit_from_scratch",
        train_module="trainers.fastervit_from_scratch",
        weights_key="fastervit_from_scratch",
        default_image_size=224,
        builder=_build_fastervit_from_scratch,
    ),
}
