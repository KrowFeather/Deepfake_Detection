...
class BasicBlock(nn.Module):
    """带两个 3x3 卷积的基础 ResNet 块。
    
    基础块实现残差连接：
    output = F(x) + x
    
    其中 F(x) 是通过两个 3x3 卷积的变换。
    这允许梯度直接通过恒等连接流动，
    有助于训练深度网络。
    """
    ...
    def __init__(
        ...
        参数:
            in_channels: 输入通道数
            out_channels: 输出通道数
            stride: 第一个卷积的步长（默认: 1）
            downsample: 残差连接的可选下采样层
                       （当 stride != 1 或通道数改变时需要）
        """
        super().__init__()
        # 第一个 3x3 卷积
...
class ResNet18(nn.Module):
    ...
    ResNet（残差网络）引入了跳跃连接，使训练非常深的网络成为可能。
    ResNet-18 总共有 18 层：
    - 1 个初始卷积层
    - 4 个残差层（每层有 2 个基础块）
    - 1 个分类器层
    ...
    架构:
    - conv1: 7x7，步长 2，64 通道
    - maxpool: 3x3，步长 2
    - layer1: 2 个块，64 通道，步长 1
    - layer2: 2 个块，128 通道，步长 2
    - layer3: 2 个块，256 通道，步长 2
    - layer4: 2 个块，512 通道，步长 2
    - avgpool + fc: 分类
    """
    ...
    def __init__(self, num_classes: int = 2) -> None:
        ...
        # 残差层：每层包含 2 个基础块
        self.layer1 = self._make_layer(64, 2, stride=1)   # 2 个块，64 通道，不下采样
        self.layer2 = self._make_layer(128, 2, stride=2)  # 2 个块，128 通道，下采样
        self.layer3 = self._make_layer(256, 2, stride=2)  # 2 个块，256 通道，下采样
        self.layer4 = self._make_layer(512, 2, stride=2)  # 2 个块，512 通道，下采样
    ...
    def _make_layer(self, out_channels: int, num_blocks: int, stride: int = 1) -> nn.Sequential:
        ...
        参数:
            out_channels: 此层的输出通道数
            num_blocks: 此层中的基础块数量
            stride: 第一个块的步长（后续块使用 stride=1）
            
        返回:
            基础块的顺序容器
        """
        downsample = None
        # 如果 stride != 1 或通道数改变，创建下采样层
    ...
    def _initialize_weights(self) -> None:
        """使用 Kaiming 初始化（适用于 ReLU 激活）初始化权重。"""
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Kaiming 初始化：适用于 ReLU 激活
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 分类器
        x = self.avgpool(x)  # 全局平均池化: (B, 512, H, W) -> (B, 512, 1, 1)
        x = torch.flatten(x, 1)  # 展平: (B, 512, 1, 1) -> (B, 512)
        x = self.fc(x)  # 分类: (B, 512) -> (B, num_classes)
        return x
...
# Output filenames
BEST_WEIGHTS_NAME: str = "ResNet18Model.pth"
...
def main() -> None:
    ...
    # Load custom ResNet18 (implemented from scratch)
